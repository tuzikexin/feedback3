...
class CFG:
    num_workers=4
    path="/kaggle/input/common-nlp-tokenizer/model_tokenizer/tiny/deberta_v3_base/"
...
class CustomModel(nn.Module):
    ...
    def feature(self, inputs):
        ...
        hs1 = outputs.hidden_states[-2]
        hs2 = outputs.hidden_states[-3]
        #weighted_pooling_embeddings = self.pool(all_hidden_states)
        #feature = torch.mean(weighted_pooling_embeddings, 1)#weighted_pooling_embeddings[:, 0]
        feature = self.pool(last_hidden_states+hs1*0.6+0.2*hs2, inputs['attention_mask'])
        #feature2 = self.pool2(last_hidden_states, inputs['attention_mask'])
...
for fold in CFG.trn_fold:
    ...
    state = torch.load(f"/kaggle/input/shuwen-tiny-model/shuwen_deberta_v3_base/shuwen_{fold}.pth",
...
submission[['text_id'] + CFG.target_cols].to_csv('./out/deberta_v3_base.csv', index=False)
